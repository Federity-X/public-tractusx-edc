...
class DidDocumentServiceDimClientExtensionTest {

    private static final String DIM_URL = "https://dim.example.com/api/v1";
    ...
    void setup(ServiceExtensionContext context) {
        ...
        context.registerService(DimOauth2Client.class, dimOauth2Client);
        context.registerService(TypeManager.class, typeManager);
    }

    @Test
    void shouldRegisterClient_whenClientTypeIsDim(ServiceExtensionContext context, ObjectFactory factory) {
        when(context.getConfig()).thenReturn(ConfigFactory.fromMap(allSettings()));

        var extension = factory.constructInstance(DidDocumentServiceDimClientExtension.class);
        extension.initialize(context);

        var registeredClient = context.getService(DidDocumentServiceClient.class);
        assertThat(registeredClient).isInstanceOf(DidDocumentServiceDimClient.class);
    }
    ...
    void shouldNotRegister_whenClientTypeIsIdentityhub(ServiceExtensionContext context, ObjectFactory factory) {
        ...
        verify(monitor).info(contains("is not set to 'dim'"));
    }
    ...
    @Test
    void shouldNotRegister_whenDimUrlMissing(ServiceExtensionContext context, ObjectFactory factory) {
        var settings = allSettings();
        settings.remove("tx.edc.iam.sts.dim.url");
        when(context.getConfig()).thenReturn(ConfigFactory.fromMap(settings));

        var extension = factory.constructInstance(DidDocumentServiceDimClientExtension.class);
        extension.initialize(context);

        verify(monitor).warning(contains("DIM URL not configured"));
    }

    private HashMap<String, String> allSettings() {
        return new HashMap<>(Map.of(
                "tx.edc.did.service.client.type", "dim",
                "tx.edc.iam.sts.dim.url", DIM_URL,

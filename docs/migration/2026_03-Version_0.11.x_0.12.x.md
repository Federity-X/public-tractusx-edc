...
In addition, a new parameter is required to select which wallet backend the self-registration feature uses:

```properties
TX_EDC_DID_SERVICE_CLIENT_TYPE="div"
```

This property (`tx.edc.did.service.client.type`) determines which `DidDocumentServiceClient` implementation is active.
Valid values are `div` (for SAP DIV) and `identityhub` (for Eclipse Tractus-X IdentityHub).
If the property is not set, no client is registered and self-registration is a no-op.

> **Breaking Change:** Previously, the DIV client activated implicitly when `tx.edc.iam.sts.div.url` was configured.
> Starting with this version, you **must** explicitly set `TX_EDC_DID_SERVICE_CLIENT_TYPE=div` for the DIV client
> to activate. Existing DIV deployments need to add this property.
...
[RemoteTokenServiceClientExtension](../../edc-extensions/dcp/tx-dcp-sts-div/src/main/java/org/eclipse/tractusx/edc/iam/dcp/sts/RemoteTokenServiceClientExtension.java):

| Old value              | New value                  | New environment variable |
|------------------------|----------------------------|--------------------------|
| `edc.iam.sts.div.url`  | `tx.edc.iam.sts.div.url`   | `TX_EDC_IAM_STS_DIV_URL` |

...
```properties
TX_EDC_DID_SERVICE_CLIENT_TYPE="dim"
```

This property (`tx.edc.did.service.client.type`) determines which `DidDocumentServiceClient` implementation is active.
Valid values are `dim` (for SAP DIM/DIV) and `identityhub` (for Eclipse Tractus-X IdentityHub).
If the property is not set, no client is registered and self-registration is a no-op.

> **Breaking Change:** Previously, the DIM client activated implicitly when `tx.edc.iam.sts.dim.url` was configured.
> Starting with this version, you **must** explicitly set `TX_EDC_DID_SERVICE_CLIENT_TYPE=dim` for the DIM client
> to activate. Existing DIM deployments need to add this property.
